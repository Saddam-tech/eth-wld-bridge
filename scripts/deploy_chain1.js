...
async function main() {
  ...
  let {
    NETWORKFEE_ID_CHAIN1,
    NETWORKFEE_FEETYPE_CHAIN1,
    WRAPPED_COIN_NAME_CHAIN1,
    WRAPPED_COIN_SYMBOL_CHAIN1,
    BRIDGE_FEERATE_CHAIN1,
    NETWORK_FEERATE_CHAIN1,
    NETWORK_FEE_AMOUNT_CHAIN1,
  } = process.env;
  ...
  const _bridge = await bridge.deploy(
    ethers.utils.parseUnits(BRIDGE_FEERATE_CHAIN1, 18), // bridge fee rate
    ethers.utils.parseUnits(NETWORK_FEERATE_CHAIN1, 18), // network fee rate
    NETWORKFEE_ID_CHAIN1, // network fee id
    _WETH.address, // network fee contract address
    NETWORKFEE_FEETYPE_CHAIN1, // network fee type
    ethers.utils.parseUnits(NETWORK_FEE_AMOUNT_CHAIN1, 18) // network fee amount
  );
