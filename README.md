...
EthereumBridge (inherited):

The contract inherits from the BridgeBase and is initialized as EthereumBridge. It is deployed to Ethereum.
...
The next step is to set the admin of the token contract on Worldland to the WorldlandBridge contract address. As the mint function is called from inside of the WorldlandBridge contract the sender is always going to be the WorldlandBridge contract address, so we send mint and burn privilege of token contract to WorldlandBridge contract.
